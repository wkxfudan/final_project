...
void Game::GameBoard::handlecollision(int i, int j)
{
	board[i][j]->upgrade();
...
bool Game::GameBoard::checkwin() const
{
	for (int i = 0; i < dimension; i++)
		for (int j = 0; j < dimension; j++)
			if (board[i][j] != NULL)
...
bool Game::GameBoard::checklose() const
{
	if (full())
		if (!movePossible())
		{
...
Game::GameBoard::GameBoard(int dim)
{
	dimension = dim;
	score = 0;
	collisionflag = false;
...
void Game::GameBoard::move(Direction dir)
{
	flaginit();
	switch (dir)
	{
...
void Game::GameBoard::print() const
{
	for (int j = 0; j < dimension; j++)
	{
		for (int i = 0; i < dimension; i++)
...
bool Game::GameBoard::full() const
{
	for (int i = 0; i < dimension; i++)
		for (int j = 0; j < dimension; j++)
			if (board[i][j] == NULL)
...
std::pair<int, int> Game::GameBoard::freeposition() const
{
...
bool Game::GameBoard::movePossible() const
{
	GameBoard cb = GameBoard(*this);
	cb.move(LEFT);
	if (cb.moveflag == true)
		return true;
	cb.move(RIGHT);
...
void Game::GameBoard::movedown(int i, int j)
{
...
void Game::GameBoard::moveup(int i, int j)
{
	bool combinflag = false;
	if (board[i][j] == NULL)
		return;
...
void Game::GameBoard::moveleft(int i, int j)
{
	bool combinflag = false;
	if (board[i][j] == NULL)
		return;
...
void Game::GameBoard::moveright(int i, int j)
{
	bool combinflag = false;
	if (board[i][j] == NULL)
		return;
